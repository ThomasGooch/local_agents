...
class ReviewAgent(BaseAgent):
    ...
    def __init__(self, model: Optional[str] = None, **kwargs):
        super().__init__(
            ...
            goal="Analyze code quality, identify issues, and suggest improvements for maintainability, security, and performance",
            model=model,
            **kwargs
        )
...
Be specific in your feedback, provide examples where helpful, and prioritize issues by their potential impact.
""",
        ])
    ...
    def _run_static_analysis(self, target: str) -> str:
        """Run static analysis tools on the target with improved error handling and categorization."""
        findings = []
        target_path = Path(target)
        
        try:
            # Python static analysis
            if target_path.suffix == '.py' or (target_path.is_dir() and self._contains_python_files(target_path)):
    ...
    def _contains_js_files(self, directory: Path) -> bool:
        ...
        return any(file.suffix in extensions for file in directory.rglob('*') if file.suffix in extensions)
    ...
    def _map_flake8_severity(self, code: str) -> Severity:
        ...
        elif code.startswith('E') and code[1] in ['1', '2', '3']:  # Indentation, whitespace, blank line
            return Severity.LOW
        ...
        for severity in [Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW, Severity.INFO]:
            ...
            if severity_findings:
                ...
                for finding in severity_findings:
                    ...
                    output_parts.append(f"  {finding.tool}: {location} - {finding.message}{rule_info}")
