...
class PlanningAgent(BaseAgent):
    ...
    def __init__(self, model: Optional[str] = None, **kwargs):
        super().__init__(
            ...
            goal="Create comprehensive, actionable implementation plans that break down complex tasks into manageable steps",
            model=model,
            **kwargs
        )
...
Make the plan detailed enough to be actionable but concise enough to be easily followed.
""",
        ])
        
        return "\n".join(prompt_parts)
    
    def plan_feature(self, feature_description: str, context: Optional[Dict[str, Any]] = None) -> TaskResult:
    ...
    def plan_bugfix(self, bug_description: str, context: Optional[Dict[str, Any]] = None) -> TaskResult:
    ...
    def plan_refactor(self, refactor_description: str, context: Optional[Dict[str, Any]] = None) -> TaskResult:
