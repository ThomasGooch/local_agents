...
class TestAgentChaining:
    ...
    def test_plan_to_code_integration(self, mock_ollama_class, mock_ollama_client_with_responses):
        ...
        code_result = coder.execute(
            "Implement the calculator based on the plan",
            code_context
        )
    ...
    def test_code_to_review_integration(self, mock_ollama_class, mock_ollama_client_with_responses):
        ...
        assert any(word in review_result.output.lower() for word in ["positive", "issues", "recommendations"])
    ...
    def test_full_agent_chain(self, mock_ollama_class, mock_ollama_client_with_responses):
        ...
        code_result = coder.execute(
            "Implement calculator based on the plan",
            code_context
        )
        assert code_result.success
...
class TestAgentContextHandling:
    ...
    @patch('local_agents.base.OllamaClient')
    def test_agent_handles_empty_context(self, mock_ollama_class, mock_ollama_client_with_responses):
    ...
    @patch('local_agents.base.OllamaClient')
    def test_agent_handles_rich_context(self, mock_ollama_class, mock_ollama_client_with_responses, sample_python_file):
