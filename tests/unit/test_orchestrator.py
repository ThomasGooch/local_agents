...
from local_agents.workflows.orchestrator import Workflow, WorkflowStep, WorkflowResult
...
class TestWorkflowOrchestrator:
    ...
    def test_custom_workflow_invalid_steps(self, workflow):
        ...
        with pytest.raises(ValueError, match="Unknown agent type: invalid"):
            workflow.create_custom_workflow(
                steps=['plan', 'invalid', 'test'],
                task='Invalid workflow'
            )
    ...
    def test_workflow_execution_time_tracking(self, workflow, mock_agents):
        ...
        assert 'seconds' in result.execution_time_formatted or 'ms' in result.execution_time_formatted
        ...
        with patch.object(PlanningAgent, '__new__', return_value=mock_agents['planner']):
            with patch.object(CodingAgent, '__new__', return_value=mock_agents['coder']):
                with patch.object(TestingAgent, '__new__', return_value=mock_agents['tester']):
                    ...
                    result = workflow.execute_workflow(
                        'feature-dev',
                        'Create feature despite coding failure'
                    )
    ...
    def test_workflow_stops_on_critical_failures(self, workflow, mock_agents):
        ...
        with patch.object(PlanningAgent, '__new__', return_value=mock_agents['planner']):
            with patch.object(CodingAgent, '__new__', return_value=mock_agents['coder']):
                ...
                result = workflow.execute_workflow(
                    'feature-dev',
                    'Create feature with critical failure'
                )
